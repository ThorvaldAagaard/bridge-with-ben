...
def card_value( card):
    # Define the values for card ranks
    rank_values = {'14': 'A', '13': 'K', '12': 'Q', '11': 'J', '10': 'T'}

    # Extract the rank and suit from the card
    rank = card.symbol[1:]

    # Get the value for the rank, use the rank itself if not in the dictionary
    rank_value = rank_values.get(rank, rank)
    
    # Combine suit and rank values to form the card's value
    return rank_value

def hand_as_string(cards):
    # Initialize an empty array for each suit
    suits = {'S': '', 'H': '', 'D': '', 'C': ''}

    # Concatenate the value for each card to its respective suit
    for card in cards:
        suit = card.symbol[0]
        suits[suit] += card_value(card)        

    # Convert the dictionary values to a list
    suit_values = list(suits.values())

    # Join the values for each suit with '.'
    result = '.'.join(suit_values)

    return result

class BotBid:

    def __init__(self, vuln, hand, models, ns, ew, score, sampler, verbose):
        self.vuln = vuln
        self.hand_str = hand_as_string(hand)
        self.hand = binary.parse_hand_f(32)(self.hand_str)
    ...
    def bid(self, auction):
        ...
        if BotBid.do_rollout(auction, candidates, hands_np):
            ...
            return BidResp(bid=candidates[0].bid, candidates=candidates, samples=samples[:20])
    ...
    def sample_hands(self, auction_so_far):
        ...
        lho_pard_rho = self.sample.sample_cards_auction(auction_so_far, turn_to_bid, self.hand, self.vuln, self.model, self.binfo_model, self.ns, self.ew,self.sample.sample_boards_for_auction)[:self.sample.sample_hands_auction]
...
class BotLead:
    ...
    def __init__(self, vuln, hand, models, ns, ew, lead_threshold, sample, verbose):
        self.vuln = vuln
        self.hand_str = hand_as_string(hand)
        self.hand = binary.parse_hand_f(32)(self.hand_str)
        self.hand52 = binary.parse_hand_f(52)(self.hand_str)
    ...
    def find_opening_lead(self, auction):
        ...
        accepted_samples, tricks = self.simulate_outcomes_opening_lead(auction, lead_card_indexes)
        ...
        if (candidate_cards[0].insta_score > 0.75):
    ...
    def simulate_outcomes_opening_lead(self, auction, lead_card_indexes):
        ...
        accepted_samples = self.sample.sample_cards_auction(auction, lead_index, self.hand, self.vuln, self.bidder_model, self.binfo_model, self.ns, self.ew, self.sample.sample_boards_for_auction_lead)[:self.sample.sample_hands_opening_lead]
...
class CardPlayer:
    ...
    def __init__(self, player_models, player_i, hand, public_hand, contract, is_decl_vuln, verbose = False):
        self.player_models = player_models
        self.model = player_models[player_i]
        self.player_i = player_i
        self.hand_str = hand_as_string(hand)
        self.public_hand_str = hand_as_string(public_hand)
